...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // 2. Circuit Breaker (KV) -- IP yang sudah terlalu sering memicu honeypot
      //    langsung dapat respons statis murah, tanpa komputasi payload/watermark.
      //    Event ini juga di-log (temuan review: sebelumnya CB block TIDAK tercatat
      //    ke Analytics Engine sama sekali, jadi operator tidak tahu CB pernah aktif).
      ...
      // 3. Context Extraction -- PENTING: path WAJIB ada di context, karena
      //    signal seperti "/etc/passwd" atau "/.env" ada di pathname, bukan
      //    query string. (Bug ini pernah bikin semua deteksi path-based gagal
      //    total di draft sebelumnya -- lihat catatan di README §Changelog.)
      const context = {
      ...
      if (evaluation.isAttack && evaluation.score >= SCORE_THRESHOLD) {
        ...
        // salt_fallback event (PRD §3.4.1): dicatat di sini, bukan di watermark.js,
      ...
      // 6. Aman -> teruskan ke origin/backend asli
      return passthrough(request, env);
    } catch (error) {
